    ...
    <div class="relative flex flex-col items-center gap-6 pb-8 text-black">
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col gap-4 w-full max-w-2xl px-4">
        ...
        <div v-if="step === 1" class="flex flex-col items-center gap-6">
          ...
          <InputTexto class="w-[300px]" _placeholder="Ej. Casa Los Pinos" v-model="hogar.nombre" />
          <p class="text-center">Sin inspiración? Intenta uno de estos nombres</p>
          <div class="flex flex-row justify-center flex-wrap gap-4">
            <Button type="button" _texto="Casa actual" class="rounded-4xl" @click="setNombre('Casa actual')" />
            <Button type="button" _texto="Oficina actual" @click="setNombre('Oficina actual')" />
            <Button type="button" _texto="Mi lugar feliz" @click="setNombre('Mi lugar feliz')" />
          </div>
        </div>
        ...
        <div v-if="step === 2" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <InputTexto _placeholder="Tipo de Propiedad" v-model="hogar.tipoPropiedad" />
          <InputTexto _placeholder="Número de Habitaciones" type="number" v-model.number="hogar.habitaciones" />
          <InputTexto _placeholder="Número de Baños" type="number" v-model.number="hogar.baños" />
        </div>
        ...
        <div v-if="step === 3" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="flex items-center gap-2">
            ...
            <input type="checkbox" v-model="hogar.calefaccion" class="accent-blue-600" />
          </div>
          <InputTexto _placeholder="Abastecimiento de Agua" v-model="hogar.abastecimientoAgua" />
          <InputTexto _placeholder="Proveedor de Internet" v-model="hogar.proveedorInternet" />
          <InputTexto _placeholder="Sistema de Seguridad" v-model="hogar.sistemaSeguridad" />
          <InputTexto _placeholder="Funciones Inteligentes" type="number"
            v-model.number="hogar.funcionesInteligentes" />
          <InputTexto _placeholder="URL de la Imagen" v-model="hogar.imgUrl" />
        </div>
      </form>
...
export default defineComponent({
  ...
  setup(_, { emit }){
    ...
    const resetForm = () => {
      ...
      hogar.direccion = '';
      hogar.tipoPropiedad = '';
      hogar.habitaciones = 0;
      hogar.baños = 0;
      hogar.calefaccion = false;
      hogar.abastecimientoAgua = '';
      hogar.proveedorInternet = '';
      hogar.sistemaSeguridad = '';
      hogar.funcionesInteligentes = 0;
      hogar.imgUrl = '';
      step.value = 1;
    };
